...
export default function TemporalPage({uniqueYears,
    ...
    */
    ...
    useEffect(() => {
        ...
        for(let [k, v] of filterMap.entries()){
            for(let val of v.keys()){
                console.log(`key: ${k}, vel: ${val}`)
            }
    ...
    */
    ...
    const nbAccidentsSelectedYears = useMemo(() => {
            return(
                idxFilterSet.size === 0 ? accidentData["an"].length : idxFilterSet.size
            )
        },
        [accidentData, idxFilterSet]
    );
    ...
    const nbHospitalizedSelectedYears = useMemo(() => {
            return(
                getIndexesSet(
                    accidentData["grav"],
                    3,
                    idxFilterSet.size === 0 ? undefined : idxFilterSet
                ).size
            )
            //return(getIndexes(
            //    validGravValues,
    ...
    const nbDeathsSelectedYears = useMemo(() => {
            ...
            return([
                {
                    ...
                    hovertemplate: `Mois: %{x}<br>`+
                                   `Nombre: %{y}<br>`+
                                   `Année: ${selectedYears[0]}-${selectedYears[1]}`,
                    //hovertext: monthList,
                    //text: monthList,
                }
            ])
            ...
            <div id="keyNumbersDiv">
                <span id="keyNumbersParentSpan">
                    <div className='keyNumbersElementDiv'>
                        <span>
                            Nombre d'accidents entre {selectedYears[0]} et {selectedYears[1]}:
                        </span>
                        <span 
                    ...
                    <div className='keyNumbersElementDiv'>
                        <span>
                            Nombre de morts entre {selectedYears[0]} et {selectedYears[1]}:
                        </span>
                        <span className='keyNumberSpan'>
                            {nbDeathsSelectedYears}
                        </span>
                        <span>
                            Représente {
                                ((nbDeathsSelectedYears/
                                nbAccidentsSelectedYears)*100).toFixed(2)
                            }% des accidents totaux.
                        </span>
                    </div>
                    <div className='keyNumbersElementDiv'>
                        <span>
                            Nombre d'hospitalisations entre {selectedYears[0]} et {selectedYears[1]}:
                        </span>
                        <span className='keyNumberSpan'>
                            {nbHospitalizedSelectedYears}
                        </span>
                        <span>
                            Représente {
                                ((nbHospitalizedSelectedYears/
